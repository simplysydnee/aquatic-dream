...
- `groupByFamily: true`: after the same dedupe and ranking, collapse entries on normalized parent phone (digits only, leading US `1` dropped). Each `FamilyGroup` is `{ parent_name, parent_phone, parent_email, source, swimmers: Array<{ swimmer_name, child_dob, parent_email, source }> }`. Entries with no phone stay their own single-swimmer group keyed by lowercased email, then by parent name, so nothing is lost. A swimmer that appears under two parent emails but the same phone lands in one group with one swimmer entry (deduped by lowercased swimmer name). The 12 cap applies to groups in this mode.
