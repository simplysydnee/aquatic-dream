// Admin-only report for the fall 2026 re-engagement announcement.
// Builds the audience and the exact rendered messages. Sends nothing, writes nothing.
import { createClient } from "npm:@supabase/supabase-js@2";
import { corsHeaders } from "npm:@supabase/supabase-js@2/cors";
import { buildFall2026List, FALL2026_KIND } from "../_shared/fall2026-outreach.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const optedOutRecipients = list.recipients.filter((r) =>
      optedOut.has(optOutPhoneKey(r.phone) ?? r.phone),
    );

    const samples = (segment: string) =>
      list.recipients.filter((r) => r.segment === segment).slice(0, 10).map((r) => r.message);
    ...
    return json({
      ...
      excluded_active_enrollment_phones: list.excludedActiveEnrollmentPhones,
      ...
      samples: { PREVIOUS: samples("PREVIOUS"), INQUIRY: samples("INQUIRY") },
    ...
    console.error("[build-fall2026-outreach]", e);
