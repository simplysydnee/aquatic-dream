  ...
  <Html lang="en" dir="ltr">
    ...
    <Body style={main}>
      <Container style={container}>
        ...
        <Text style={text}>
          {childName ? `${childName}'s` : "Your swimmer's"} swim lessons are starting
          {dueDate ? ` on ${dueDate}` : ' soon'}! Please complete the session fee payment
          {amountDue ? ` of ${amountDue}` : ''} to secure their spot.
        </Text>
        {sessionInfo && (
...
export const template = {
  ...
  subject: (data: Record<string, any>) =>
    `Session Fee Payment Due${data.childName ? ` for ${data.childName}` : ''} — ${SITE_NAME}`,
